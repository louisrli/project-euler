...
int main() {
	...
	for (unsigned int i = 12; i < kLimit + 1; i++) {
		...
		if (abundant_nums.find(i) != abundant_nums.end() )
			continue;
		...
		if (i < (primes.sum_of_divisors(i) - i)) {
			...
			for (int j = 2 * i; j < kLimit + 1; j += i) {
				abundant_nums.insert(j);
				is_sum[j] = true;
			}
		}
	}
	
	// Find all numbers that are sums of abundant numbers by adding them together
	for (set<int>::iterator iter_first = abundant_nums.begin(); iter_first != abundant_nums.end(); iter_first++) {
		for (set<int>::iterator iter_second = iter_first; *iter_second < (kLimit + 1) - *iter_first; iter_second++) {
			int x = *iter_first;
			int y = *iter_second;
